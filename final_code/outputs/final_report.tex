...
\begin{enumerate}
    \item \textbf{Feature importance.} Momentum dominates across cities (Fig.~\ref{fig:shap-beeswarm}). In Tokyo wards, the four-quarter moving average drives 72\% of LightGBM SHAP weight and the one-quarter lag another 16\%; Sendai wards are similar but give more mass to quarter-over-quarter growth (12\%). Mesh Random Forest plots show trend features above 80\% (Tokyo MA4Q 60\%, QoQ 19\%; Sendai MA4Q 58\%, QoQ 24\%), with hedonic indices and missingness flags only in the long tail. LSTM SHAP confirms it mostly re-learns the same trend cues, explaining why it lags trees on sparse meshes.
...
\begin{minipage}{0.24\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shap_plots/wardtokyo_lightgbm_val_beeswarm.png}
    \caption*{Tokyo Ward LightGBM}
\end{minipage}\hfill
\begin{minipage}{0.24\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shap_plots/wardsendai_lightgbm_val_beeswarm.png}
    \caption*{Sendai Ward LightGBM}
\end{minipage}\hfill
\begin{minipage}{0.24\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shap_plots/meshtokyo_randomforest_val_beeswarm.png}
    \caption*{Tokyo Mesh RF}
\end{minipage}\hfill
\begin{minipage}{0.24\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shap_plots/meshsendai_randomforest_val_beeswarm.png}
...
\begin{minipage}{0.24\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shap_plots/wardtokyo_torchlstm_val_beeswarm.png}
    \caption*{Tokyo Ward LSTM}
\end{minipage}\hfill
\begin{minipage}{0.24\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shap_plots/wardsendai_torchlstm_val_beeswarm.png}
    \caption*{Sendai Ward LSTM}
\end{minipage}\hfill
\begin{minipage}{0.24\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shap_plots/meshtokyo_torchlstm_val_beeswarm.png}
    \caption*{Tokyo Mesh LSTM}
\end{minipage}\hfill
\begin{minipage}{0.24\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shap_plots/meshsendai_torchlstm_val_beeswarm.png}
    \caption*{Sendai Mesh LSTM}
\end{minipage}

\caption{SHAP beeswarm plots split by city and level. Momentum dominates everywhere; Sendai wards lean slightly more on quarter-over-quarter growth, while Tokyo meshes show higher reliance on moving-average trend and lags. LSTMs emphasise the same trend cues and add little beyond momentum, especially on sparse Sendai meshes.}
\label{fig:shap-beeswarm}
\end{figure}

\noindent Sendai SHAP patterns are noisier because only five wards feed the model; repeated LightGBM ``no further splits'' warnings during training reflect that sparsity, reinforcing our choice to lean on Random Forest for Sendai deployment.
