...
def run():
    ...
    print("Skip LSTM runs with SKIP_LSTM=1 to keep the demo fast.")
    ...
    if not skip_lstm:
        ...
        except Exception as exc:  # missing torch or other deps
            print(f"Skipping LSTM pipeline (import failed): {exc}")
        else:
