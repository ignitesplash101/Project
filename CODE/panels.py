...
def build_mesh_panel(main_df: pd.DataFrame, mesh_panel_raw: pd.DataFrame, ward_panel: pd.DataFrame, hedonic: dict) -> pd.DataFrame:
    ...
    mesh_panel["Mesh250m"] = mesh_panel["Mesh250m"].astype(str)
    ...
    if not mesh_hedonic_full.empty and "Mesh250m" in mesh_hedonic_full.columns:
        mesh_hedonic_full["Mesh250m"] = mesh_hedonic_full["Mesh250m"].astype(str)
    mesh_hedonic_train = hedonic.get("mesh_train", pd.DataFrame())
    if not mesh_hedonic_train.empty and "Mesh250m" in mesh_hedonic_train.columns:
        mesh_hedonic_train["Mesh250m"] = mesh_hedonic_train["Mesh250m"].astype(str)
